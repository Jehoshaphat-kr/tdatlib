...
class treemap:
    ...
    def __init__(self):
        self.t_prev, self.t_curr = (kst - timedelta(365 * 2)).strftime("%Y%m%d"), kst.strftime("%Y%m%d")

        m = self.market = market()
        self.theme, self.etf, _ = m.theme, m.etf_group, m.performance
        self.icm = pd.concat(objs=[m.icm, m.etf_stat[['시가총액', '종가']]], axis=0, ignore_index=False)
        return
    ...
    def set_option(self, category:str, index:str=str()):
        ...
        if category == 'WICS':
            self.__category, self.__name = self.market.wics, '전체'
        elif category == 'WI26':
            self.__category, self.__name = self.market.wi26, '전체'
        elif category == 'THEME':
    ...
    def to_js(self):
        ...
        yy, mm, dd = self.t_curr[:4], self.t_curr[4:6], self.t_curr[6:]
        suffix = codecs.open(
            filename=os.path.join(archive.map_js), mode='r', encoding='utf-8'
        ).read()

        syntax = f'document.getElementsByClassName("date")[0].innerHTML="{yy}년 {mm}월 {dd}일 종가 기준";'
        _dir = os.path.join(archive.root, 'deploy/js')
        _cnt = 1
        _js = os.path.join(_dir, f"{self.t_curr[2:]}MM-r{_cnt}.js")
        ...
        _frm.to_csv(r'./test.csv', encoding='euc-kr', index=True)
    ...
    def tickers(self) -> list:
        ...
        # 코스피200(1028), 코스피 중형주(1003), 코스피 소형주(1004), 코스닥150(2203), 코스닥 중형주(2003)
            ...
            depo = self.market.i_deposit.copy()
            self.__tickers += depo[depo['지수코드'].astype(str) == self.__idx].index.tolist()
    ...
    def baseline(self) -> pd.DataFrame:
        ...
        if self.__cat.startswith('WI') or self.__cat == 'THEME':
            ...
            if self.__cat.startswith('WI') and not self.__idx:
                ...
                depo = self.market.i_deposit.copy()
                samples = depo[depo['지수코드'].astype(str) == self.__idx]
