from tdatlib.archive.deprecated.viewer.stock.value.core import chart
from tdatlib.archive.deprecated.viewer.tools import save
...
class value(chart):
    ...
    def saveall(self, path:str=str()):
        ...
        for n, (fig, name) in enumerate(proc):
            ...
            save(fig=fig, tag=self.tag, filename=f'F{str(n + 1).zfill(2)}_{name}', path=path)
        return
