...
class treemap(object):
    ...
    def baseline(self) -> pd.DataFrame:
        ...
        if not hasattr(self, '__baseline'):
            ...
            perf = self.market.get_returns(tickers=baseline.index)
        ...
        for t, lim in limiter.items():
            ...
            if pos.empty:
                ...
                if pos_bin[0] == pos_bin[1]:
                    pos_bin[0] = 0.01
                if pos_bin[1] == pos_bin[2]:
                    pos_bin[1] = (pos_bin[0] + pos_bin[1]) / 2
                p_color = pd.cut(pos[t], bins=pos_bin, labels=scale[4:], right=True)
                p_color.fillna(scale[4], inplace=True)
                objs.append(p_color)
    ...
    def __calc_post(self):
        """ 지도 데이터 후처리 """
        def rename(x):
            return x['종목명'] + "*" if x['종목코드'] in self.market.get_deposit('kosdaq') else x['종목명']
        def reform_price(x):
