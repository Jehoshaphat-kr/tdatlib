...
from tdatlib.archive.deprecated.dataset.stock import KR as stock
import pandas as pd
import numpy as np
...
class compare(object):
    ...
    def __init__(self, tickers:list or np.array, period:int=3, endate:str=str()):
    ...
    @property
    def rel_price(self) -> pd.DataFrame:
        """
        가격[종가] 데이터
        :return:
    ...
    def rel_yield(self) -> pd.DataFrame:
        ...
        for name, ticker in zip(self.names, self.tickers):
            rel = self.__getattribute__(f'__{ticker}').ohlcv_rr.copy()
    ...
    def rel_drawdown(self) -> pd.DataFrame:
        ...
        objs = dict()
        for name, ticker in zip(self.names, self.tickers):
            rel = self.__getattribute__(f'__{ticker}').ohlcv_dd.copy()
            for col in rel.columns:
                objs[(col, name)] = rel[col]
        return pd.concat(objs=objs, axis=1)
    ...
    def rel_mfi(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n:self.__getattribute__(f'__{t}').ohlcv_ta.volume_mfi for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_rsi(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.momentum_rsi for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_stoch(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.momentum_stoch for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_cci(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.trend_cci for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_roc(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.momentum_roc for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_vortex(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.trend_vortex_ind_diff for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_bb(self) -> pd.DataFrame:
        ...
        """
        return pd.concat(
            objs={n: self.__getattribute__(f'__{t}').ohlcv_ta.volatility_bbp for n, t in zip(self.names, self.tickers)},
            axis=1
        )
    ...
    def rel_sharpe_ratio(self) -> pd.DataFrame:
        ...
        for name, ticker in zip(self.names, self.tickers):
            ...
            objs[(name, 'cagr')] = attr.ohlcv_cagr.loc[ticker].tolist()
            objs[(name, 'risk')] = attr.ohlcv_volatility.loc[ticker].tolist()
    ...
    def rel_profit(self) -> pd.DataFrame:
        ...
        profit = pd.DataFrame()
        for name, ticker in zip(self.names, self.tickers):
            data = self.__getattribute__(f'__{ticker}').basis_annual[['영업이익률', '배당수익률', 'ROA', 'ROE']]
            objs = dict()
            for i in data.index:
    ...
    def rel_growth(self) -> pd.DataFrame:
        ...
        growth = pd.DataFrame()
        for name, ticker in zip(self.names, self.tickers):
            stat = self.__getattribute__(f'__{ticker}').basis_annual
    ...
    def rel_multiple(self) -> pd.DataFrame:
        ...
        objs = list()
        for name, ticker in zip(self.names, self.tickers):
            nps = self.__getattribute__(f'__{ticker}').basis_nps
            stat = self.__getattribute__(f'__{ticker}').basis_annual
            key = [_ for _ in ['매출액', '순영업수익', '이자수익', '보험료수익'] if _ in stat.columns][0]
...
if __name__ == "__main__":
    ...
    t_series = compare(tickers=t_tickers, period=5)
    # print(t_series.rel_price)
